...
boolean vsnprintfFailOnce = FALSE;
int vsnprintfErrorCode = -1;
 
int __wrap_vsnprintf(char *str, size_t size, const char *format, va_list ap)
{

  if (vsnprintfFailOnce)
  {
    vsnprintfFailOnce = FALSE;
    return vsnprintfErrorCode;
  }
  else
  {
...
void Test_CF_vsnprintf_success(void)
{
  size_t bufSize = 20;
  char buffer[bufSize];
  uint8 someArg = 42;
  uint16 truncErrorId = 9999;
  uint16 copyErrorId = 8888;
  uint32 current_event_count = Ut_CFE_EVS_GetEventQueueDepth();

  CF_vsnprintf(buffer, bufSize, copyErrorId, truncErrorId, "The answer is %d", someArg);

  UtAssert_True(Ut_CFE_EVS_GetEventQueueDepth() == current_event_count, "There was an error that should not have occurred and events were received that should not have been.");
}

void Test_CF_vsnprintf_fail_format_greater_than_buffer(void)
{
  // Arrange
  size_t bufSize = 1;
  char buffer[bufSize];
  char expectedErrorMessage[57];
  char format[17];
  uint8 someArg = 42;
  uint16 truncErrorId = 9999;
  uint16 copyErrorId = 8888;
  ...
  sprintf(format, "The answer is %%d");
  sprintf(expectedErrorMessage, "Truncated message 'The answer is %%d', lost %zu characters", (strlen(format) - bufSize));
  
  //Act
  CF_vsnprintf(buffer, bufSize, copyErrorId, truncErrorId, format, someArg);

  //Assert
  UtAssert_True(Ut_CFE_EVS_GetEventQueueDepth() == current_event_count + 1, "Event queue should have increased by 1, but did not.");
  UtAssert_EventSent(truncErrorId, CFE_EVS_ERROR, expectedErrorMessage, "An event was not found with all 3 values.");
}

void Test_CF_vsnprintf_fail_cannot_write_event_for_failed_copy(void)
{
  // Arrange
  size_t bufSize = 1;
  char buffer[bufSize];
  char expectedSysLogMessage[57];
  char format[17];
  uint8 someArg = 42;
  uint16 truncErrorId = 9999;
  uint16 copyErrorId = 8888;
  uint32 current_event_count = Ut_CFE_EVS_GetEventQueueDepth();
  uint32 current_sys_log_count = Ut_CFE_ES_GetSysLogQueueDepth();
  
  sprintf(format, "The answer is %%d");
  sprintf(expectedSysLogMessage, "CF:Error sending vsnprintf copy error event:RC=0x%08X\n", CFE_EVS_APP_NOT_REGISTERED);
  
  Ut_CFE_EVS_SetReturnCode(UT_CFE_EVS_SENDEVENT_INDEX, CFE_EVS_APP_NOT_REGISTERED, 1);
  
  //Act
  CF_vsnprintf(buffer, bufSize, copyErrorId, truncErrorId, format, someArg);

  //Assert
  UtAssert_True(Ut_CFE_EVS_GetEventQueueDepth() == current_event_count, "Event queue could not have increased or decreased but did.");
  UtAssert_True(Ut_CFE_ES_GetSysLogQueueDepth() == current_sys_log_count + 1, "Sys log queue should have increased by 1, but did not.");
  Ut_CFE_ES_SysLogWritten(expectedSysLogMessage);
}

void Test_CF_vsnprintf_fail_vsnprintf_lib_call_fails(void)
{
  // Arrange
  size_t bufSize = 20;
  char buffer[bufSize];
  char expectedErrorMessage[62];
  char format[17];
  uint8 someArg = 42;
  uint16 truncErrorId = 9999;
  uint16 copyErrorId = 8888;
  uint32 current_event_count = Ut_CFE_EVS_GetEventQueueDepth();
  vsnprintfFailOnce = TRUE;
  
  sprintf(format, "The answer is %%d");
  sprintf(expectedErrorMessage, "Error (%d), unable to copy format to buffer: %s", vsnprintfErrorCode, format);
  ...
  //Assert
  UtAssert_True(Ut_CFE_EVS_GetEventQueueDepth() == current_event_count + 1, "Event queue should have increased by 1, but did not.");
  UtAssert_EventSent(copyErrorId, CFE_EVS_ERROR, expectedErrorMessage, "An event was not found with all 3 values.");
}

void Test_CF_vsnprintf_fail_cannot_write_event_for_truncated_fail(void)
{
  // Arrange
  size_t bufSize = 1;
  char buffer[bufSize];
  char expectedSysLogMessage[62];
  char format[17];
  uint8 someArg = 42;
  uint16 truncErrorId = 9999;
  uint16 copyErrorId = 8888;
  ...
  sprintf(format, "The answer is %%d");
  sprintf(expectedSysLogMessage, "CF:Error sending vsnprintf truncated error event:RC=0x%08X\n", CFE_EVS_APP_NOT_REGISTERED);
...
void CF_AddTestCase(void)
{
    ...
    UtTest_Add(Test_CF_vsnprintf_success, CF_Setup, CF_TearDown, "Test_CF_vsnprintf_success");
    UtTest_Add(Test_CF_vsnprintf_fail_format_greater_than_buffer, CF_Setup, CF_TearDown, "Test_CF_vsnprintf_fail_format_greater_than_buffer");
    UtTest_Add(Test_CF_vsnprintf_fail_cannot_write_event_for_failed_copy, CF_Setup, CF_TearDown, "Test_CF_vsnprintf_fail_cannot_write_event_for_failed_copy");
    UtTest_Add(Test_CF_vsnprintf_fail_vsnprintf_lib_call_fails, CF_Setup, CF_TearDown, "Test_CF_vsnprintf_fail_vsnprintf_lib_call_fails");
    UtTest_Add(Test_CF_vsnprintf_fail_cannot_write_event_for_truncated_fail, CF_Setup, CF_TearDown, "Test_CF_vsnprintf_fail_cannot_write_event_for_truncated_fail");
#if 0
